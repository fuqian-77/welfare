	$(window).resize(function(){
		...
		pmyGrid1=myLoadTextGridData(pmyGrid1);
		pmyGrid1=myCellFocus(pmyGrid1,0,0);
	});

	function myTextGrid(pmyGrid1){//定义网格
		//单元格名称RiCj。e.g.  r1c0  ^r只读
		...
		pmyGrid1.fixedfieldcol=-1;
		pmyGrid1.keyfieldfieldcol=-1;
		for (var j=0; j<pmyGrid1.columns.length; j++){
			if (pmyGrid1.columns[j].name==pmyGrid1.keyfield || pmyGrid1.columns[j].name==pmyGrid1.fixedfield){
				pmyGrid1.fixedwidth=pmyGrid1.columns[j].length;
				pmyGrid1.fixedfieldcol=j;
				if (pmyGrid1.columns[j].name==pmyGrid1.keyfield) pmyGrid1.keyfieldcol=j;
				break;
			}
		}	
		//屏幕分成上下两块
		...
		pmyGrid1.rowcount=pmyGrid1.rows;
	...
	function myGridKeyEvent(pmyGrid1,e,id){  //按键处理
		...
		var xtype=[];
		var i=0;
		//nsole.log(input.getCursorPosition());
		if (key==13 || key==40 || key==38  || key==35 || key==36){ //  || key==35 || key==36){  //38--up  40--down
			...
			myFocus('cell_tmp');
			pmyGrid1=myCellFocus(pmyGrid1,row,col);
	...
	function myGridLocate(pmyGrid1,n){  //定位到某一行后选中checkbox
		...
		pmyGrid1=myCellFocus(pmyGrid1,n,0);		
	...
	function myGridCheckBoxChange(pmyGrid1,id,e){ 
		...
		if (pmyGrid1.select=='single'){
			if (id=='hr_checkbox'){  //总复选框
				...
				row=$("#"+id).attr('xrow');
				pmyGrid1=myCellFocus(pmyGrid1,row,0);
			}
		}else{  //多选
			if (id=='hr_checkbox'){  //总复选框
	...
	function myAppendGridRow(pmyGrid1){  //追加一行
		...
		pmyGrid1=myCellFocus(pmyGrid1,pmyGrid1.rows,0);
		pmyGrid1.rows=pmyGrid1.rows+1;
		var top=(pmyGrid1.rowheight+0)*(pmyGrid1.rows-1)+0;
		$("#"+pmyGrid1.id).scrollTop(top);  //光标移动到新增行
		return pmyGrid1;
	}
	...
	function myCreateGridRows(pmyGrid1,n0,n){ //新增一行rrrrrrr
		...
		for (var i=n0; i<n0+n; i++){
			...
			if (pmyGrid1.fixedfieldcol<0) left=0;
			for (var j=0; j<pmyGrid1.columns.length; j++){
				var id='r'+row+'c'+j;  //产生单元格
				...
				str2='<div id="'+id+'" class="textgrid" onclick="pmyGrid1=myCellFocus(pmyGrid1,'+row+','+j+')" class="textgrid" style="position:absolute; text-align:'+align+'; padding:0px 2px 0px 6px; line-height:'+(pmyGrid1.rowheight*1)+'px; height:'+(pmyGrid1.rowheight)+'px; width:'+(w)+'px; top:'+(top)+'px; left:'+(left)+'px;"></div>\n';
		...
		for (j=0;j<pmyGrid1.columns.length;j++){
			if (pmyGrid1.columns[j].name==pmyGrid1.keyfield || pmyGrid1.columns[j].name==pmyGrid1.fixedfield) left=0;
			else left+=1*pmyGrid1.columns[j].length;
		}
		myAddHeaderScrollbar();
		return pmyGrid1;		
	}

	function myAddHeaderScrollbar(){  //
		//在标题hbar2中添加一个小方块,先判断是否存在vertical滚动条
		var top=0;
		var left=1*(pmyGrid1.rownumberwidth+pmyGrid1.checkboxwidth+1)-2+2;
		if (pmyGrid1.fixedfieldcol<0) left=0;  //?????
		for (j=0;j<pmyGrid1.columns.length;j++){
		...
		if (sw>0){
			...
			if ($('#'+id+'_rownox').length>0) $('#'+id+'_rownox').remove();
	...
	function myCreateGridHeader(pmyGrid1){  // hhhhhhhhheader
		...
		if (pmyGrid1.fixedfieldcol<0) left=-4;
		var j=0;
		for (var j=0; j<pmyGrid1.columns.length; j++){
			if (pmyGrid1.columns[j].name==pmyGrid1.keyfield || pmyGrid1.columns[j].name==pmyGrid1.fixedfield ){
			...
			if (type=='n'){  //numberfield
				...
				myNumberField(id,parent,'',0,top,left,pmyGrid1.rowheight*1+1,w-4+4,len,pmyGrid1.columns[j].dec,0,min,max,'');
		...
		$("#"+pmyGrid1.id).append($(str));
		eval(css);
		$("#cell_0").textbox("textbox").bind("keyup",function(e){
			//console.log($("#cell_0").getCursorPosition()); 
		});
		//设置只读
		for (var j=0; j<pmyGrid1.columns.length; j++){
			var id='cell_'+j;
			if (pmyGrid1.columns[j].readonly==1){
				mySetReadonly(id,true);
			}
		}	
		return pmyGrid1;		
	}
	...
	function myInsertGridRow(pmyGrid1,row){
		...
		for (var i=pmyGrid1.rows-1;i>=row;i--){
			...
			c1.attr('xrow',i);
			c2.attr('xrow',i-1);
			//if (pmyGrid1.keyfieldfieldcol>=0) 
			mySetValue('r_key'+i,myGetValue('r_key'+(i-1)));  //关键字移动
			for (j=0;j<pmyGrid1.columns.length;j++){
				c1='r'+i+'c'+j;
				c2='r'+(i-1)+'c'+j;
	...
	function myDeleteGridRow(pmyGrid1,row){//ddddddddd
		...
		for (var i=row;i<pmyGrid1.rows-1;i++){
			...
			c1.prop("checked", c2.is(':checked'));
			//if (pmyGrid1.keyfieldfieldcol>=0) 
			mySetValue('r_key'+i,myGetValue('r_key'+(1*i+1)));
			for (j=0;j<pmyGrid1.columns.length;j++){
				c1='r'+i+'c'+j;
				c2='r'+(i*1+1)+'c'+j;
		...
		myAddHeaderScrollbar(); //放在这个位置，否则光标会移到下一行
	...
	function mySaveGridRows(pmyGrid1){  //ssssssssssave
		...
		pmyGrid1=myCellFocus(pmyGrid1,0,0);
		$("#"+pmyGrid1.id).scrollLeft(0);
		$("#"+pmyGrid1.id).scrollTop(0);
		if (pmyGrid1.showsavemessage) myMessageShow('共'+k+'行数据已经保存成功！',240);
		return pmyGrid1;
	}

	
	function myTextGridColumns(pmyGrid1){
		//[^r#230%n@r]资源名称/resourcename; //^r只读
		var xcolumns=[];
		...
		for (var i=0;i<xfields.length;i++){
			...
			if (p1>=0 && p2>p1){
				...
				var x0=ss.indexOf('^r');
				if (x0>=0){
					xcolumns[i].readonly=1;
					ss=ss.replace('^r','');  //删除该描述
				}
				var x1=ss.indexOf('@');
				if (x1>=0){
	...
	function myLoadTextGridData(pmyGrid1){   //lllllllload
		...
		$.ajax({     
			...
			success: function(data) {
				...
				var i=0;
				//增加不足的行
				for (i=pmyGrid1.rows; i<records.length;i++){
					pmyGrid1=myAppendGridRow(pmyGrid1);
					pmyGrid1.rowindex=0;
				}
				//数据库值赋值到行
				for (i=0; i<records.length; i++){
					pmyGrid1.data[i]=[]; //二维数组arr[i]=new array()
					for (var j=0; j<pmyGrid1.columns.length; j++){
						c1='r'+i+'c'+j;
						c2=eval("records[i]."+pmyGrid1.columns[j].name);
						mySetValue(c1,c2);
						pmyGrid1.data[i][j]=c2;
					}
				...
				for (var i=n1; i<n2; i++){
					...
					for (j=0;j<pmyGrid1.columns.length;j++){
						c1='r'+i+'c'+j;
						$('#r'+i+'c'+j).remove();  //删除最后一行的所有控件
					}
				}
				*/
				pmyGrid1.rowcount=Math.max(records.length,pmyGrid1.rows);
		...
		pmyGrid1=myCellFocus(pmyGrid1,pmyGrid1.rowindex,0);
		$("#"+pmyGrid1.id).scrollLeft(0);
		$("#"+pmyGrid1.id).scrollTop(0);
		return pmyGrid1;
	}
	...
	function myCellFocus(pmyGrid1,row,col){
		...
		if (row0!=row && row>=0){  //rown0<0时不赋值
			...
			if (col0==pmyGrid1.fixedfieldcol && keyvalue!='' && pmyGrid1.columns[col0].readonly==0 && pmyGrid1.columns[col0].name==pmyGrid1.keyfield){
				var flag=false;
				for (var i=0;i<pmyGrid1.rows;i++){
	...
	function myCopyGridRow(pmyGrid1){
		pmyGrid1.copyvalues=[];
		for (var j=0; j<pmyGrid1.columns.length; j++){
			var id='cell_'+j;
			pmyGrid1.copyvalues.push(myGetValue(id));
		}
		return pmyGrid1;
	}

	function myPasteGridRow(pmyGrid1){
		if (pmyGrid1.copyvalues!=undefined){
			for (var j=0; j<pmyGrid1.columns.length; j++){
				var id='cell_'+j;
				if (pmyGrid1.columns[j].readonly==0){
					mySetValue(id,pmyGrid1.copyvalues[j]);
				}
			}
		}
	}

	function myBlankGridRow(pmyGrid1){  //清空可编辑列
		for (var j=0; j<pmyGrid1.columns.length; j++){
			var id='cell_'+j;
			if (pmyGrid1.columns[j].readonly==0){
				mySetValue(id,'');
	...
	function myTextGridUpdatedFlag(pmyGrid1){
		flag=0;
		if (pmyGrid1.data.length==0 || pmyGrid1.rows==0){
			return 0;
		}
		if (pmyGrid1.rows!=pmyGrid1.data.length || pmyGrid1.data.length==0) flag=1;
		if (pmyGrid1.deletedrows!=''){
			var tmp=pmyGrid1.deletedrows.split('	');
			for (var i=1; i<tmp.length; i++){
				if (tmp[i]!=''){
					flag++;
		...
		for (var i=0; i<pmyGrid1.rows; i++){
			for (var j=0; j<pmyGrid1.columns.length; j++){
				var c1='r'+i+'c'+j;
				if (myGetValue(c1)!=pmyGrid1.data[i][j]){
					//console.log(c1,pmyGrid1.data[i][j],flag);
					flag++;
					break;
				}
			}
		...
		if (flag>0){
			if (pmyGrid1.autorowsave) pmyGrid1=mySaveGridRows(pmyGrid1);
			else{
				$.messager.confirm({
					title:'系统提示', 
					msg:myConfirmMessage('是否保存本次操作数据？'),
					width:280,
					height:160,
					fn:function(r){
						if (r){
							pmyGrid1=mySaveGridRows(pmyGrid1);
							myMessageEvent('confirm','yes');
						}else{
							myMessageEvent('confirm','no');
						}
					}	
				});	
			}
	...
	(function ($, undefined) {
	    $.fn.getCursorPosition = function() {
	        var el = $(this).get(0);
	        var pos = 0;
	        if('selectionStart' in el) { //non IE
	            pos = el.selectionStart;
	            //el.selectRange.text="xxxx";
	            //el.selectRange.select();
